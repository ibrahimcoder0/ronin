...
if ( ! function_exists( 'ronin_header_add_to_cart_fragment' ) ) {
   function ronin_header_add_to_cart_fragment( $fragments ) {
       ob_start();
       ?>
       <span class="cart__count" id="mt-cart-item">
           <?php echo esc_html( WC()->cart->cart_contents_count ); ?>
       </span>
       <?php
       $fragments['#mt-cart-item'] = ob_get_clean();

       return $fragments;
   }
}
add_filter( 'woocommerce_add_to_cart_fragments', 'ronin_header_add_to_cart_fragment' );




// ronin_product_sidebar_filter before
function ronin_product_sidebar_before($output){
   $output = '<div class="mt-shop-widget-product">';

   return $output;
}
add_filter('woocommerce_before_widget_product_list','ronin_product_sidebar_before');
// ronin_product_sidebar_filter after
function ronin_product_sidebar_after($output){
   $output = '</div>';

   return $output;
}
add_filter('woocommerce_after_widget_product_list','ronin_product_sidebar_after');

// ronin_sale_percentage
function ronin_sale_percentage(){
   global $product;
   $output = '';
   $icon = esc_html__("-",'ronin');

   if ( $product->is_on_sale() && $product->is_type( 'variable' ) ) {
      $percentage = ceil(100 - ($product->get_variation_sale_price() / $product->get_variation_regular_price( 'min' )) * 100);
      $output .= '<div class="product-percentage-badges"><span class="mt-product-details-offer">'. $icon . $percentage.'%</span></div>';
      ...
      $output .= '<span class="mt-product-details-offer">'.$icon . $percentage.'%</span>';
      $output .= '</div>';
   }
   return $output;
}
...
// ronin_product_details
function ronin_product_details(){
   global $product;
   global $post;
   global $woocommerce;
   ...
   ?>
      <div class="mt-shop-details__right-warp">
            ...
            <h3 class="mt-shop-details__title-sm"><?php the_title(); ?></h3>
            
            <div class="mt-shop-details__ratting">
               <?php woocommerce_template_single_rating(); ?>
            </div>
            <div class="mt-shop-details__price">
               <span><?php woocommerce_template_single_price(); ?></span>
               <?php if( $product->is_on_sale() && !$product->is_type( 'grouped' ) ) : ?>
               <span class="red-color"><?php echo ronin_sale_percentage(); ?></span>
               <?php endif; ?>   
            </div>
            <div class="mt-shop-details__text-2">
               <?php woocommerce_template_single_excerpt(); ?>
            </div>
            <div class="mt-shop-details__quantity-wrap d-flex align-items-center">
            ...
            <div class="product__details-share">
               <?php ronin_product_social_share(); ?>
            </div>
      </div>
   <?php
}

add_action('woocommerce_single_product_summary','ronin_product_details');




// product add to cart button
function ronin_wooc_add_to_cart( $args = array() ) {
    global $product;
        ...
        if ( $product ) {
            $defaults = array(
                ...
                'class'      => implode(
                    ...
                    array_filter(
                        array(
                            'mt-product-action-btn-2 mt-product-add-cart-btn cart-button icon-btn button',
         ...
         if( $product->is_type( 'simple' ) ){
            $btntext = esc_html__("Add to Cart",'ronin');
         } elseif( $product->is_type( 'variable' ) ){
            $btntext = esc_html__("Select Options",'ronin');
         } elseif( $product->is_type( 'external' ) ){
            $btntext = esc_html__("Buy Now",'ronin');
         } elseif( $product->is_type( 'grouped' ) ){
            $btntext = esc_html__("View Products",'ronin');
         }
         else{
            $btntext = esc_html__("Add to Cart",'ronin');
         } 
        ...
        echo sprintf( '<a href="%s" data-quantity="%s" class="%s" %s>%s</a>',
            ...
            '                  <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  </svg> <span class="mt-product-tooltip mt-product-tooltip-right">'.$btntext.'</span>'
        );
}
...
?>
   ...
   <div class="mt-product-item-2 ">
      <div class="mt-product-thumb-2 ">
         <a href="<?php the_permalink(); ?>">
            <?php the_post_thumbnail(); ?>
         </a>
         <!-- product action -->
         <div class="mt-product-action-2 mt-product-action-blackStyle">
            <div class="mt-product-action-item-2 d-flex flex-column">
               <?php ronin_wooc_add_to_cart(); ?>
               <?php if ( function_exists( 'woosq_init' ) ) : ?>
               <div type="button" class="mt-product-action-btn-2 mt-product-quick-view-btn">
                  <?php echo do_shortcode('[woosq]'); ?>                                                                       
                  <span class="mt-product-tooltip mt-product-tooltip-right"><?php echo esc_html__('Quick View','ronin'); ?></span>
               </div>
               <?php endif; ?>
               <?php if ( function_exists( 'woosw_init' ) ) : ?>
               <div type="button" class="mt-product-action-btn-2 mt-product-add-to-wishlist-btn">
                  <?php echo do_shortcode('[woosw]'); ?>                                                                              
                  <span class="mt-product-tooltip mt-product-tooltip-right"><?php echo esc_html__('Add To Wishlist','ronin'); ?></span>
               </div>
               <?php endif; ?>
               <?php if ( function_exists( 'woosc_init' ) ) : ?>   
               <div type="button" class="mt-product-action-btn-2 mt-product-add-to-compare-btn">
                  <?php echo do_shortcode('[woosc]'); ?>                                                                           
                  <span class="mt-product-tooltip mt-product-tooltip-right"><?php echo esc_html__('Add To Compare','ronin'); ?></span>
               </div>
               <?php endif; ?>
      ...
      <div class="mt-product-content-2 ">
         <?php if ( !empty( $categories ) ): ?>
         <div class="mt-product-tag-2">
         <?php 
            $html = '';
         ...
         <?php endif; ?>
         <h3 class="mt-product-title-2">
            <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
         </h3>
         <div class="mt-product-rating-icon">
            <?php woocommerce_template_loop_rating(); ?>
         </div>
         <div class="mt-product-price-wrapper-2">
...
add_action( 'wp_footer' , 'ronin_custom_quantity_fields_script' );

// custom_quantity_fields_script
function ronin_custom_quantity_fields_script(){
    ?>
    ...
    jQuery( function( $ ) {
        ...
        $( document.body ).on( 'click', '.mt-cart-plus, .mt-cart-minus', function() {
